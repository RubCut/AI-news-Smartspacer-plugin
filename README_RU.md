...
блокирует начиная с API 28. Трафик к самому устройству — `localhost`, `127.0.0.1`
и эмуляторный алиас `10.0.2.2` — разрешён сразу, через точечный
...
- **Сервер на телефоне** (Termux, UserLAnd) или в эмуляторе:
  `http://localhost:11434/v1`. Включать ничего не нужно.
- **Сервер на другом устройстве** — адрес в локальной сети не является loopback,
  поэтому обычный HTTP на него останется запрещённым, пока в настройках таргета
  не включить **Разрешить незащищённые соединения**. Переключатель действует на
  один таргет и по умолчанию выключен, а включение спрашивает дважды: в диалоге
  прямым текстом написано, что ключ, промпт и новости пойдут по сети в открытом
  виде и их можно прочитать и подменить на пути (отказ в диалоге возвращает
  переключатель в выключенное состояние), и пока опция включена, это предупреждение
  висит красным под переключателем.
- **В обоих случаях** TLS или проброс порта оставляют трафик шифрованным:
  `https://…` для сервера с сертификатом либо

  ```bash
  adb reverse tcp:11434 tcp:11434   # затем используйте http://localhost:11434/v1
  ```

Если платформа отказала в запросе, в настройках покажется понятный переводимый
текст с именем хоста, а не системное исключение.

### Проверка без модели

[`tools/mock-ai-server.py`](tools/mock-ai-server.py) — замена локального сервера,
только стандартная библиотека Python: он отдаёт список моделей и две готовые
новости, так что видно, что запрос дошёл и обновление прошло целиком, — без Ollama
и без API-ключа.

```bash
python3 tools/mock-ai-server.py --port 11434
adb reverse tcp:11434 tcp:11434
# настройки → базовый URL http://localhost:11434/v1 → Проверить ключ → Загрузить модели → Сохранить
```

Если указать вместо этого адрес в локальной сети и не делать `adb reverse`, тот же
скрипт проверяет путь с включённым переключателем «Разрешить незащищённые
соединения».
...
- без шифрования запросы уходят только там, где ты сам так включил: loopback-адреса
  либо локальный сервер с тумблером «Разрешить незащищённые соединения». Облачные
  провайдеры всегда ходят по HTTPS, это не отключается;
...
├── PlainHttp.kt              # HTTP по сокету, для тумблера «незащищённые»
