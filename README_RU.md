...
(например `http://localhost:11434/v1`), ключ можно оставить пустым.

### Локальные серверы

Ollama, llama.cpp, LM Studio и подобные им отдают обычный HTTP, который Android
блокирует начиная с API 28. Плагин разрешает незашифрованный трафик только к
самому устройству — `localhost`, `127.0.0.1` и эмуляторный алиас `10.0.2.2` —
через точечный
[network security config](app/src/main/res/xml/network_security_config.xml),
поэтому все облачные провайдеры по-прежнему работают только по HTTPS.

- **Сервер на телефоне** (Termux, UserLAnd): `http://localhost:11434/v1`.
- **Сервер на компьютере** — либо включите TLS и используйте `https://…`,
  либо пробросьте порт по USB, и сервер станет «локальным»:

  ```bash
  adb reverse tcp:11434 tcp:11434   # затем используйте http://localhost:11434/v1
  ```

Если платформа отказала в незашифрованном запросе, в настройках покажется
понятный текст, а не системное исключение.
...
1. Откройте релизы.
2. Поставьте/Обновите с помощью последнего релиза.
